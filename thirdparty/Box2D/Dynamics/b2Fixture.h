...
public:
    ...
    /// @param p a point in world coordinates.
    void ComputeDistance(const b2Vec2& p, float32* distance, b2Vec2* normal,
    ...
    /// the mass of the body. You must call b2Body::ResetMassData to update the
    /// body's mass.
...
protected:
    ...
    // constructor/destructor because the destructor cannot access the allocator
    // (no destructor arguments allowed by C++).
...
inline void b2Fixture::ComputeDistance(const b2Vec2& p, float32* d, b2Vec2* n,
                                       int32 childIndex) const {
